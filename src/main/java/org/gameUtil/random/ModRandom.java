...
/**
 * A ModRandom object generates a stream of random numbers from 0 to some
...
public class ModRandom {
    ...
    /**
     * A ModRandom object is thought of as a bag of marbles of different types,
     ...
     * in a controlled spread (Default mod of 1, see ModRandom(int count, double mod))
     * 
     * @param count the upper non-exclusive bound of random integer generation
     *         <p> count must be at least 1
     */
    public ModRandom(int count) {
        this(count, 1);
    }
     ...
     */
    public ModRandom(int count, double mod) {
        if (mod <= 0)
            throw new IllegalArgumentException("ModRandom initialized with modularity " + mod);
        if (count < 1)
            throw new IllegalArgumentException("ModRandom initialized to count " + count
                    + "arguments");
